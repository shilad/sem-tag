package org.semtag.relate;

import com.typesafe.config.Config;
import gnu.trove.set.TIntSet;
import gnu.trove.set.hash.TIntHashSet;
import org.semtag.dao.ConceptDao;
import org.semtag.dao.DaoException;
import org.semtag.dao.DaoFilter;
import org.semtag.model.Tag;
import org.semtag.model.concept.Concept;
import org.wikapidia.conf.Configuration;
import org.wikapidia.conf.ConfigurationException;
import org.wikapidia.conf.Configurator;
import org.wikapidia.core.lang.Language;
import org.wikapidia.core.lang.LocalId;
import org.wikapidia.core.lang.LocalString;
import org.wikapidia.sr.LocalSRMetric;
import org.wikapidia.sr.SRResult;
import org.wikapidia.sr.SRResultList;

/**
 * @author Ari Weiland
 */
public class WikapidiaRelator implements ConceptRelator {

    private final ConceptDao helperDao;
    private final Language language;
    private final LocalSRMetric srMetric;

    protected WikapidiaRelator(ConceptDao helperDao, Language language, LocalSRMetric srMetric) {
        this.helperDao = helperDao;
        this.language = language;
        this.srMetric = srMetric;
    }

    public ConceptDao getHelperDao() {
        return helperDao;
    }

    public Language getLanguage() {
        return language;
    }

    public LocalSRMetric getSrMetric() {
        return srMetric;
    }

    @Override
    public double relatedness(Concept x, Concept y) throws DaoException {
        return relatedness(x.getConceptId(), y.getConceptId());
    }

    @Override
    public double relatedness(int xId, int yId) throws DaoException {
        try {
            SRResult result = srMetric.similarity(
                    new LocalId(language, xId).asLocalPage(),
                    new LocalId(language, yId).asLocalPage(),
                    false);
            return result.getScore();
        } catch (org.wikapidia.core.dao.DaoException e) {
            throw new DaoException(e);
        }
    }

    @Override
    public RelatedResultList mostRelated(Concept obj, int maxResults) throws DaoException {
        return mostRelated(obj.getConceptId(), maxResults, 0);
    }

    @Override
    public RelatedResultList mostRelated(Concept obj, int maxResults, double threshold) throws DaoException {
        return mostRelated(obj.getConceptId(), maxResults, threshold);
    }

    @Override
    public RelatedResultList mostRelated(int id, int maxResults) throws DaoException {
        return mostRelated(id, maxResults, 0);
    }

    @Override
    public RelatedResultList mostRelated(int id, int maxResults, double threshold) throws DaoException {
        Iterable<Concept> concepts = helperDao.get(new DaoFilter());
        TIntSet validIds = new TIntHashSet();
        for (Concept c : concepts) {
            validIds.add(c.getConceptId());
        }
        SRResultList results;
        try {
            results = srMetric.mostSimilar(
                    new LocalId(language, id).asLocalPage(),
                    maxResults,
                    validIds);
        } catch (org.wikapidia.core.dao.DaoException e) {
            throw new DaoException(e);
        }
        return buildRelatedResultList(results, maxResults, threshold);
    }

    @Override
    public RelatedResultList mostRelated(Tag tag, int maxResults) throws DaoException {
        return mostRelated(tag, maxResults, 0);
    }

    @Override
    public RelatedResultList mostRelated(Tag tag, int maxResults, double threshold) throws DaoException {
        Iterable<Concept> concepts = helperDao.get(new DaoFilter());
        TIntSet validIds = new TIntHashSet();
        for (Concept c : concepts) {
            validIds.add(c.getConceptId());
        }
        SRResultList results;
        try {
            results = srMetric.mostSimilar(
                    new LocalString(language, tag.getNormalizedTag()),
                    maxResults,
                    validIds);
        } catch (org.wikapidia.core.dao.DaoException e) {
            throw new DaoException(e);
        }
        return buildRelatedResultList(results, maxResults, threshold);
    }

    private RelatedResultList buildRelatedResultList(SRResultList results, int maxResults, double threshold) {
        RelatedResultList list = new RelatedResultList(maxResults, threshold);
        for (SRResult r : results) {
            if (r.getId() > -1) {
                list.add(new RelatedResult(r.getId(), r.getScore()));
            }
        }
        list.lock();
        return list;
    }

    @Override
    public double[][] corelatedness(Concept[] objs) throws DaoException {
        return corelatedness(getVector(objs));
    }

    @Override
    public double[][] corelatedness(Concept[] xObjs, Concept[] yObjs) throws DaoException {
        return corelatedness(getVector(xObjs), getVector(yObjs));
    }

    @Override
    public double[][] corelatedness(int[] ids) throws DaoException {
        try {
            return srMetric.cosimilarity(ids, language);
        } catch (org.wikapidia.core.dao.DaoException e) {
            throw new DaoException(e);
        }
    }

    @Override
    public double[][] corelatedness(int[] xIds, int[] yIds) throws DaoException {
        try {
            return srMetric.cosimilarity(xIds, yIds, language);
        } catch (org.wikapidia.core.dao.DaoException e) {
            throw new DaoException(e);
        }
    }

    private int[] getVector(Concept[] objs) {
        int[] ids = new int[objs.length];
        for (int i=0; i<objs.length; i++) {
            ids[i] = objs[i].getConceptId();
        }
        return ids;
    }

    public static class Provider extends org.wikapidia.conf.Provider<ConceptRelator> {
        public Provider(Configurator configurator, Configuration config) throws ConfigurationException {
            super(configurator, config);
        }

        @Override
        public Class getType() {
            return ConceptRelator.class;
        }

        @Override
        public String getPath() {
            return "sem-tag.related.concept";
        }

        @Override
        public WikapidiaRelator get(String name, Config config) throws ConfigurationException {
            if (!config.getString("type").equals("wikapidia")) {
                return null;
            }
            return new WikapidiaRelator(
                    getConfigurator().get(ConceptDao.class, config.getString("conceptDao")),
                    Language.getByLangCode(config.getString("lang")),
                    getConfigurator().get(LocalSRMetric.class, config.getString("metric"))
            );
        }
    }
}
